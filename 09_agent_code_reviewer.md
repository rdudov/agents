...
3. **Отчёт о тестировании** — файл `{artifacts_dir}/reports/test_report_task_X_Y.md`
4. **Код проекта** — существующий код для проверки совместимости
5. **Документация проекта** — для проверки актуализации
6. **Каталог артефактов пайплайна (`artifacts_dir`)** — путь к артефактам процесса
...
Допустимые значения `review_decision`: `approved` | `rework_required` | `rejected`
...
### ✅ Код УТВЕРЖДЁН
- Все требования из описания задачи реализованы
- Все E2E тесты прошли (или заблокированы окружением — не кодом)
...
**Тесты, заблокированные окружением:** если тест не прошёл из-за отсутствия инфраструктуры (нет БД, нет сервиса, нет переменных окружения), а не из-за ошибки в коде — это **не критичное замечание** и не является основанием для отклонения кода. Зафиксируй такие тесты в поле `blocked_by_environment` в JSON и в разделе "Некритичные замечания".
Но если тестовые данные в целом нерепрезентативны или проверочный контур фиктивен, это является проблемой качества тестов и должно быть отмечено как минимум важным, а при явном нарушении критериев приёмки — критичным замечанием.
