...
6. Вести файл status.md и обновлять его НЕМЕДЛЕННО после каждого из событий:
...
Перед тем как сообщить о завершении, сверить status.md с фактическими артефактами (файлы ТЗ, архитектуры, плана, кода), отчётами о тестировании и текущим состоянием репозитория. При любом расхождении — сначала обновить status.md, затем отвечать.
...
1. ОБЯЗАТЕЛЬНЫЙ CHECKPOINT (выполни до отчёта):
   - Сверь status.md с фактическими артефактами: файлы ТЗ, архитектуры, плана, кода существуют?
   - Сверь status.md с отчётами о тестировании: все задачи, отмеченные как выполненные, имеют test_report?
   - Проверь наличие раздела "Заблокировано окружением" — если блокировки есть, они отражены в status.md?
   - При любом расхождении — сначала обнови status.md, затем переходи к следующему шагу.
2. Собери статистику:
