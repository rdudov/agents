...
**ВАЖНО:** Используй venv проекта (найди его в корне проекта или смотри документацию). Перед запуском приложения или сервисов проверь, что они не запущены (ps aux, systemctl status и т.п.).
...
В начале ответа верни машиночитаемый JSON-блок — оркестратор использует его для обновления status.md без анализа текста:
